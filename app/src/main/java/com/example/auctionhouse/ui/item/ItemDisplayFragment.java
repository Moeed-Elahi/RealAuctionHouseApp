...
public class ItemDisplayFragment extends Fragment {
    ...
    private String PID, timeDown, dateDown;
    private ImageView productImage, imageView;
    private Button backButton, bidButton, removeButton;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        backButton = root.findViewById(R.id.button10);
        ...
        lastBid.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                DatabaseReference dbref = FirebaseDatabase.getInstance().getReference().child("Users").child(lastBid.getText().toString().substring(13));
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (!currentdate.after(date)) {
                            ...
                            productTimeRemaining.setText("Auction is over!");
                            bidButton.setEnabled(false);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
        ...
        productsRef.child(pid).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    if (products.getUserLastBid().equals(Prevalent.currentOnlineUser.getEmail())) {
                        lastBid.setTextColor(R.color.userBid);
                    } else {
                        lastBid.setTextColor(R.color.notUserBid);
                    }
                    lastBid.setText("Last bid by: " + products.getUserLastBid());
                    setTimeRemaining();
                }
            }
    ...
    private void showImage() {
        ...
        settingsDialog.setContentView(getLayoutInflater().inflate(R.layout.image_product
                , null));
        imageView = settingsDialog.findViewById(R.id.image_product);
